...
def test_mobilenet():
    ...
    train_ds, test_ds, class_names = data_load("D:/dejahu/new_data1/train",
                                              "D:/dejahu/new_data1/val", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("models/mobilenet_y.h5")
    # model.summary()
    # 测试
    ...
    for test_batch_images, test_batch_labels in test_ds:
        ...
        test_batch_pres_max
        test_batch_labels_max = np.argmax(test_batch_labels, axis=1 = np.argmax(test_batch_pres, axis=1)
...
def test_cnn():
    ...
    train_ds, test_ds, class_names = data_load("D:/dejahu/new_data1/train",
                                              "D:/dejahu/new_data1/val", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("models/cnn_y.h5")
    # model.summary()
    # 测试
    loss, accuracy = model.evaluate(test_ds)
    # 输出结果
